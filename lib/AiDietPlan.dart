...
class QuizPage extends StatefulWidget {
  ...
  QuizPage({Key? key, this.initialSection}) : super(key: key);

  @override
  _AiDietPlanQuizState createState() => _AiDietPlanQuizState();
}

class _AiDietPlanQuizState extends State<QuizPage> with TickerProviderStateMixin {
  int _currentIndex = -1;
    ...
    setState(() {
      switch (title) {
        ...
        case 'Preferences':
          ...
          break;
        case 'Consent':
          _currentIndex = 8; // Setup Complete (you may want to create a separate consent page)
          break;
        default:
          _currentIndex = 0;
      }
      ...
      child: ElevatedButton(
        ...
        child: Row(
          children: [
            ...
            Text('${(progress * 100).toInt()}%', style: TextStyle(color: Colors.orange)),
          ],
        ),
      ),
    );
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () => _currentIndex == -1 ? Navigator.pop(context) : setState(() => _currentIndex = -1),
        ),
        title: Text(
          'Ai Diet Plan',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 24),
            ...sectionProgress.entries.map((entry) => _buildSectionButton(entry.key, entry.value)).toList(),
          ],
        ),
      ),
    );
    ...
    return SingleChildScrollView(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            if (_currentIndex == 0)
              _buildPhysicalProfileSection1(),
            if (_currentIndex == 1)
              _buildPhysicalProfileSection2(),
            if (_currentIndex == 2)
              _buildLifestyleProfileSection(),
            if (_currentIndex == 3)
              _buildAllergiesSection(),
            if (_currentIndex == 4)
              _buildEatingHabitsSection(),
            if (_currentIndex == 5)
  ...
  Widget _buildPhysicalProfileSection1() {
    return _buildSection(
      ...
      Column(
        children: [
          _buildSelector(['20', '21', '22', '23', '24'], selectedAge, (value) => setState(() => selectedAge = value)),
          SizedBox(height: 16),
          _buildSelector(['male', 'female'], selectedSex, (value) => setState(() => selectedSex = value)),
        ],
      ),
    );
  }
    ...
    return _buildSection(
      ...
      Column(
        children: [
          Row(
            children: [
              Expanded(child: _buildSelector(['3', '4', '5', '6', '7'], selectedFeet, (value) => setState(() => selectedFeet = value))),
              SizedBox(width: 16),
              Expanded(child: _buildSelector(['7', '8', '9', '10', '11'], selectedInches, (value) => setState(() => selectedInches = value))),
            ],
          ),
          SizedBox(height: 16),
          _buildSelector(['64', '65', '66', '67', '68'], selectedWeight, (value) => setState(() => selectedWeight = value)),
        ],
      ),
    );
  }
  ...
  Widget _buildLifestyleProfileSection() {
    return _buildSection(
      ...
      _buildSelector(['Sedentary', 'Lightly Active', 'Moderately Active', 'Very Active', 'Extremely Active'],
    ...
    return _buildSection(
      ...
      Column(
        ...
        children: [
          ...
          _buildSelector(['1', '2', '3', '4', '5'], selectedMeals, (value) => setState(() => selectedMeals = value)),
          SizedBox(height: 16),
          Text('Do you drink Coffee/Tea', style: TextStyle(color: Colors.grey)),
          _buildSelector(['rarely', 'sometimes', 'very frequently'], selectedCoffeeTea, (value) => setState(() => selectedCoffeeTea = value)),
          SizedBox(height: 16),
          Text('Do you consume Alcohol/cigarettes/tobacco', style: TextStyle(color: Colors.grey)),
          _buildSelector(['no', 'yes'], selectedAlcohol, (value) => setState(() => selectedAlcohol = value)),
          SizedBox(height: 16),
          Text('Do you eat out often?', style: TextStyle(color: Colors.grey)),
          _buildSelector(['no', 'yes'], selectedEatOut, (value) => setState(() => selectedEatOut = value)),
        ],
      ),
    );
  }
    ...
    return _buildSection(
      ...
      Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text('Any existing medical conditions (diabetes, hypertension, etc.)?'),
          SizedBox(height: 16),
  ...
  Widget _buildHealthGoalsSection() {
    return _buildSection(
      ...
      Column(
        ...
        children: [
          Text('Weight Goals'),
          _buildSelector(['Weight Loss', 'Weight Gain', 'Maintain'], weightGoal, (value) => setState(() => weightGoal = value)),
          SizedBox(height: 16),
          Text('Fitness Goals'),
          _buildSelector(['Muscle Building', 'Fat Loss', 'Endurance'], fitnessGoal, (value) => setState(() => fitnessGoal = value)),
          SizedBox(height: 16),
          Text('Any target timeline?'),
          _buildSelector(['1 Month', '2 Months', '3 Months', '6 Months'], targetTimeline, (value) => setState(() => targetTimeline = value)),
          SizedBox(height: 16),
          Text('Late-night snacking habits'),
          _buildSelector(['Yes', 'No'], lateNightSnacking, (value) => setState(() => lateNightSnacking = value)),
        ],
      ),
    );
  }
    ...
    return _buildSection(
      ...
      Column(
        ...
        children: [
          ...
          _buildSelector(['Daily', 'Weekly', 'Bi-weekly', 'Monthly'], checkInFrequency, (value) => setState(() => checkInFrequency = value)),
          SizedBox(height: 16),
          Text('Level of autonomy'),
          _buildSelector(['20%', '40%', '60%', '80%', '100%'], autonomyLevel, (value) => setState(() => autonomyLevel = value)),
          SizedBox(height: 16),
          Text('Feedback medium'),
          _buildSelector(['InApp', 'Email', 'SMS'], feedbackMedium, (value) => setState(() => feedbackMedium = value)),
          SizedBox(height: 16),
          Text('Late-night snacking habits'),
          _buildSelector(['Yes', 'No'], lateNightSnacking, (value) => setState(() => lateNightSnacking = value)),
        ],
  ...
  Widget _buildSection(String title, String question, String subtext, Widget image, Widget content) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        Text(title, style: TextStyle(color: Colors.grey)),
        SizedBox(height: 16),
        Center(child: Text(question, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold), textAlign: TextAlign.center)),
        SizedBox(height: 8),
        Center(child: Text(subtext, style: TextStyle(color: Colors.grey), textAlign: TextAlign.center)),
        SizedBox(height: 24),
        Center(child: image),
        SizedBox(height: 24),
        content,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 32),
          Text('AI Diet Plan', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
          SizedBox(height: 16),
          Text('Setup Complete', style: TextStyle(fontSize: 18)),
          SizedBox(height: 16),
          Icon(Icons.check_circle, color: Colors.green, size: 48),
          SizedBox(height: 24),
          Text('AI is preparing your diet plan, please wait', textAlign: TextAlign.center),
          SizedBox(height: 24),
          if (_isLoading && _animation != null)
            ...
            Text('Your diet plan is ready!', style: TextStyle(fontSize: 18, color: Colors.green)),
        ],
      ),
    );
  }

  Widget _buildSelector(List<String> options, String selected, Function(String) onChanged) {
    return Container(
  ...
  Widget _buildChip(String label) {
    return FilterChip(
      label: Text(label),
      backgroundColor: Color(0xFFFFF0E6),
      selected: selectedAllergies.contains(label) || medicalConditions.contains(label),
